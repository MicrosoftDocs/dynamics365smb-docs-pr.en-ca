...
ms.locfileid: "8808968"
---
# <a name="synchronize-items-and-inventory"></a>Synchronize Items and Inventory
...
## <a name="to-define-item-synchronizations"></a>To define item synchronizations
...
|Description|Combines extended text and attributes if corresponding toggles in the Shopify shop card are enabled. Respects language code.|Not used.|
...
## <a name="run-item-synchronization"></a>Run item synchronization
...
### <a name="image-synchronization-remarks"></a>Image synchronization remarks
...
* For price calculation, it's important to have a value in the **Default Customer Template** field. [!INCLUDE[prod_short](../includes/prod_short.md)] uses the value of the **GST/HST Business Group** field to calculate the price including GST/HST. You might want to create a customer price group where you select the **Price including GST/HST** field and specify the relevant value in the **GST/HST Bus. Posting Gr. (Price)** field.
* Enter a **Currency Code** if your online shop uses different currency than $. The specified currency must have exchange rates configured. If your online shop uses same currency as [!INCLUDE[prod_short](../includes/prod_short.md)], leave the field empty.
