...
Enter a **currency code** if your online shop uses a different currency than the local currency ($). The specified currency must have exchange rates configured. If your online shop uses the same currency as [!INCLUDE[prod_short](../includes/prod_short.md)], leave the field empty. 
...
In the **Tax area source** field, you can set the priority on how to select tax area code or GST/HST business posting group based on address. The imported Shopify order contains information about taxes, but the taxes get recalculated when you create the sales document so it's important that the GST/HST/tax settings are correct in [!INCLUDE[prod_short](../includes/prod_short.md)]. For more information about taxes, see [Set Up Taxes for the Shopify Connection](setup-taxes.md).
...
* You can assign the **Sell-to Customer No.** and **Customer No.** directly on the **Shopify Orders** page by choosing a customer from the list of existing customers.
...
* If it's **By Info**, the connector tries to find the existing customer by ID first and then by the address information. If the customer isn't found, the connector creates a new customer.

> [!NOTE]  
> The connector uses information from the address and creates the customer in [!INCLUDE[prod_short](../includes/prod_short.md)]. The sell-to customer is the same as the customer.
