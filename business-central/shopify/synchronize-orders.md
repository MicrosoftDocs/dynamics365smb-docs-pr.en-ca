...
ms.locfileid: "9361402"
---
# <a name="synchronize-and-fulfill-sales-orders"></a>Synchronize and Fulfill Sales Orders
...
In the **Tax area source** field, you can set the priority on how to select tax area code or GST/HST business posting group based on address. This step is relevant for countries with sales tax as well as those with GST/HST. Learn more at [Tax remarks](synchronize-orders.md#tax-remarks).
...
## <a name="run-the-order-synchronization"></a>Run the order synchronization
...
The imported Shopify order contains information about taxes, but the taxes get recalculated when you create the sales document so it's important that the GST/HST/tax settings are correct in [!INCLUDE[prod_short](../includes/prod_short.md)].

- Multiple product GST/HST/tax rates. For example, some product categories are liable for reduced tax rates. Those items must exist in [!INCLUDE[prod_short](../includes/prod_short.md)] and be mapped to Shopify products. Otherwise, with automatic creation of missing items, the GST/HST product posting group is used.

- Address-dependent tax rates. Use the **Tax area priority** field together with the **Customer Templates** table to overwrite standard logic that fills in the **Tax Area Code** in the sales document. The **Tax area priority** field specifies the priority used to determine the information about the country/region and Province/State. Then the corresponding record in the Shopify customer templates is found and the **Tax Area Code**, **Tax Liable** and **GST/HST Bus. Posting Group** are used when a sales document is created.

- Price including Tax. The **Prices including Tax**/**Prices including GST/HST** field in the created sales document doesn't depend on the customer, but on the **Customer Template** from the **Shopify Shop Card** page or Customer template per country.
...
## <a name="synchronize-shipments-to-shopify"></a>Synchronize shipments to Shopify
