...
- If the currency code is blank
  ...
  All transactions in the bank account are in the local currency ($) for the current company. If a transaction is made to the account in another currency, the amounts are posted to the account in LCY based on the relevant currency exchange rate. Any cheques issued from this account must be issued in $. If the bank account is used in a journal, the journal line will automatically inherit the blank currency code.  
