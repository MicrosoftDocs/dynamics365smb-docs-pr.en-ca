...
|---------|---------|---------|
|**Microsoft 365**|Everyone sends email from a shared mailbox in Exchange Online.|When all messages come from the same department, for example, your sales organization sends messages from a sales@cronus.com account. This requires that you set up a shared mailbox in the Microsoft 365 admin centre. For more information, see [Shared mailboxes](/Exchange/collaboration/shared-mailboxes/shared-mailboxes.md).|
...
## <a name="set-up-reusable-email-texts-and-layouts-for-sales-and-purchase-documents"></a>Set Up Reusable Email Text and Layouts for Sales and Purchase Documents
You can use reports to include key information from sales and purchase documents in text for emails. This procedure describes how to set up the **Sales - Invoice** report for posted sales invoices, but the process is similar for other reports.
...
5. Choose the **Email Body Layout Description** field, and then select a layout from the list.
    ...
    Report layouts define both the style and the content of the text in the email, including text such as a greeting or instructions that precede the document information. If your organization has many layouts, you can see all available report layouts if you choose the **Select from full list**.
