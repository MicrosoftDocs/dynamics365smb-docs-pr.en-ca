...
|**Max. GST/HST Difference Allowed**|The maximum amount allowed for GST/HST differences in this currency. For more information, see [Correcting GST/HST Amounts Manually in Sales and Purchase Documents](finance-work-with-vat.md#correcting-vat-amounts-manually-in-sales-and-purchase-documents). This field might not be visible by default. It can be retrieved by personalizing the page.|
|**GST/HST Rounding Type**|Specifies the rounding method for correcting GST/HST amounts manually in sales and purchase documents. This field might not be visible by default. It can be retrieved by personalizing the page.|
...
You cannot link currency codes to general ledger accounts because amounts on general ledger accounts are in $. If you have a bank loan in CAD and place deposits in a bank account in SEK, you can keep track of these accounts by setting up bank accounts in CAD and SEK. With posting groups, you can link the accounts to the relevant general ledger accounts. In the general ledger, the value of the amounts is shown in $.

You can enter a currency code on a general journal line and post the line to a general ledger account. The relevant exchange rate is used to convert the amount to $ before it is posted to the general ledger account.  
