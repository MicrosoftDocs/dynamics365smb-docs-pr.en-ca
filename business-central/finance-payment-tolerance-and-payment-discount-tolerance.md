...
Use payment tolerances so that every outstanding amount has a set maximum allowed payment tolerance. If the payment tolerance is met, the payment amount is analysed. If the payment amount is an underpayment, then the outstanding amount is fully closed by the underpayment. A detailed ledger entry is posted to the payment entry so that no remaining amount is left on the applied invoice entry. If the payment amount is an overpayment, then a new detailed ledger entry is posted to the payment entry so that no remaining amount is left on the payment entry.
...
> [!IMPORTANT]  
> When you enable the **Adjust for Payment Discount** field on the **GST/HST Posting Setup** page, the GST/HST amount is considered as it relates to the **Payment Tolerances** and **Payment Discounts** amounts, and GST/HST will be reduced for both of transaction amounts if they exist. The system can't be configured to use GST/HST reducing only for one type of transaction.  
