...
The **My Job Queue** part on your Role Centre shows the job queues entries that you have started, but which are not yet finished. By default, the part is not visible, so you have to add it to your Role Centre. For more information, see [Personalize Your Workspace](ui-personalization-user.md).  
...
### <a name="schedule-synchronization-between-prod_short-and-prod_short"></a>Schedule synchronization between [!INCLUDE[prod_short](includes/prod_short.md)] and [!INCLUDE[prod_short](includes/cds_long_md.md)]
