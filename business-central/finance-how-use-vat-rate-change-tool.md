---
title: Managing Tax Rate changes | Microsoft Docs
description: learn how to sue the Tax Rate Change tool for Dynamics 365 Business Central.
author: andregu
...
ms.locfileid: "5380947"
---
# <a name="managing-vat-rate-changes"></a>Managing Tax Rate Changes

Tax rates can change depending on local legislation. Any change in tax impacts your data in [!INCLUDE[prod_short](includes/prod_short.md)] whether or not the tax rate is lowered, raised, or removed. Tax is connected to many entities in [!INCLUDE[prod_short](includes/prod_short.md)], such as customers, vendors, items, resources, item charges, and general ledger accounts. Changes in tax rates usually happen at a specific date, from which point you will need to have changed the tax setup, posting groups etc. to make sure new sales orders and purchase orders are created with the new tax rate.

## <a name="changing-vat-rates"></a>Changing tax rates

The optimal approach to manage a tax rate change is to fully post and close open orders and other documents before the tax rate switch date, to make sure these are not impacted by the change. This is the cleanest approach that allows you to start up new orders and documents with the new tax rate.

The following approach is suggested to manage a tax rate change
...
2. Create the new tax setup.  
3. Make the tax switch on entities (relevant customers, vendors, items, and so on).  
4. At the tax rate switch date you create new documents that will use the new rate.  


> [!NOTE]  
> We are currently updating the Tax Rate Change tool. The functionality mentioned below may not match the functionality in your environment. The update will take place before July 1st 2020 and will not be a regular monthly update. Instead, all environments will be updated automatically (hotfix). When this update is complete, this message will no longer appear.  

## <a name="the-vat-rate-change-tool"></a>The Tax Rate Change tool

The Tax Rate Change tool can assist with conversion of Tax Rates on master data, journals and orders, to some extent. This is useful if you want to convert tax on master data more easily or if you have orders that you cannot close before the switch date and will be processed over a longer period of time, crossing the tax rate switch date. There are certain restrictions and limitations in the Tax Rate Change tool that applies.

## <a name="understanding-the-vat-rate-conversion-process-and-limitations"></a>Understanding the tax rate conversion process and limitations
...
On order lines, the unit price will be updated for all lines of type Item and Resource, if using prices incl. tax for the item. For other line types it is possible to decide whether or not the unit price should be updated.
...
1. Choose the ![Lightbulb that opens the Tell Me feature](media/ui-search/search_small.png "Tell me what you want to do") icon, enter **Tax Rate Change Setup**, and then choose the related link.  
...
3. On the **Prices Incl. Tax** FastTab, choose which line types on orders for which you want to update unit prices. Unit prices on lines of type Item and Resource will always be updated.

### <a name="to-set-up-product-posting-group-conversion"></a>To set up product posting group conversion

1. Choose the ![Lightbulb that opens the Tell Me feature](media/ui-search/search_small.png "Tell me what you want to do") icon, enter **Tax Rate Change Setup**, and then choose the related link.  
2. On the **Tax Rate Change Setup** page, choose either the **Tax Prod. Posting Group Conv.** or **Gen Prod. Posting Group Conv.** action.  
...
1. Choose the ![Lightbulb that opens the Tell Me feature](media/ui-search/search_small.png "Tell me what you want to do") icon, enter **Tax Rate Change**, and then choose the **Tax Rate Change Setup** link.  
...
5. After the conversion is complete, choose the **Tax Rate Change Log Entries** action to view the results of the conversion.  
