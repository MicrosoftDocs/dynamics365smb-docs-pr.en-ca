...
2. On the **General** FastTab, select or enter field information as necessary. [!INCLUDE[tooltip-inline-tip](includes/tooltip-inline-tip_md.md)] The following table describes some of the key fields.
   ...
   | **Tax No. Based On** | Specifies the customer or vendor code based on which the GST/HST number is taken for the Intrastat report.  |
   | **Company Tax No. on File** | Specifies how the company's GST/HST registration number is exported to the Intrastat file.  |
   | **Vendor Tax No. on File** | Specifies how a vendor's GST/HST registration number is exported to the Intrastat file.  |
   | **Customer Tax No. on File** | Specifies how a customer's GST/HST registration number is exported to the Intrastat file.  |
   | **Get Partner GST/HST** | Specifies which type of Intrastat report line the partner's GST/HST registration number is updated from. Depending on your local requirements, you can choose receipt lines only, shipment lines only, or both types of lines. |
...
3. On the **Default Transactions** FastTab, select or enter field information as necessary. [!INCLUDE[tooltip-inline-tip](includes/tooltip-inline-tip_md.md)] The following table describes some of the key fields.
   ...
   | **Default Private Person VAT No.** | Specifies the default private person GST/HST number if the private person must have a dedicated GST/HST number on the Intrastat report. |
   | **Default 3-Party Trade VAT No.** | Specifies the default three-party trade GST/HST number if you don't have the GST/HST number. |
...
If you want to change how Intrastat works, and the default configuration isn't enough, you can customize the system by extending the standard features. If you need to further change the Intrastat behaviour, you can develop your own codeunits. When you create codeunits, you need to make additional changes to use them. To configure the system to use your own objects, follow these steps.

1. Select the ![Lightbulb that opens the Tell Me feature.](media/ui-search/search_small.png "Tell me what you want to do") icon, enter **GST/HST Reports Configuration**, and then select the related link.
2. On the **GST/HST Reports Configuration** page, add a new line.
3. In the **GST/HST Report Type** field, select **Intrastat Report**.
4. In the **GST/HST Report Version** field, specify the version of the report.
...
Before you can include a vendor in Intrastat reporting, enter their information on the **Vendor Card** page. For example, specify a **Country/Region Code** value and a **GST/HST Registration No.** value.
...
Before you can include a customer in Intrastat reporting, enter their information on the **Customer Card** page. For example, you must specify a **Country/Region Code** value and a **GST/HST Registration No.** value.
